...
int intervalo = 5000; /// valor de en milis del intervalo

int maquinaEstadosAlarma;
#define VERDE 0
#define AMARILLO 1
#define ROJO 2
...
void loop() {
  ...
  switch (maquinaEstadosAlarma) {
    ...
    case ROJO:
      ...
      if (estadoBotonOk == no_presionado) {  //es al revez en este boton
        maquinaEstadosAlarma = ROJO_SIN_BUZZER;
      }
      if (lecturaTemperatura < VALOR_UMBRAL_TEMP) {
        maquinaEstadosAlarma = AMARILLO;
      }
      break;
  ...
  switch (estado) {
    ...
    case MENU:
      //Pantalla 0: Para ver los valores leidos de temperatura, humedad y luz
      //Print del display
    ...
    case CAMBIO_HUM:  //Pantalla 2: Para cambiar el valor umbral de hunedad
      ...
      //Lo que tiene que suceder en este estado
      VALOR_UMBRAL_HUM = lecturaBotonAbajo(VALOR_UMBRAL_HUM, uno);
      VALOR_UMBRAL_HUM = lecturaBotonArriba(VALOR_UMBRAL_HUM, uno);
...
void fun_entra (void)
{
  ...
  datos_struct[indice_entra].T1 = lecturaTemperatura; /// leeo los datos //aca va la funcion de cada sensor
  ...
  datos_struct[indice_entra].Alarma = alarma;
  indice_entra++;
